! DESCRIPTION
!   Solve the mean-field BHZ model
!
...
program bhz_2d
  ...
  real(8),dimension(:),allocatable            :: Wtk,xarray

  real(8)                                     :: chern,z2,tol
  real(8)                                     :: mh,lambda,Jh,Uloc
  ...
  call parse_input_variable(Uloc,"ULOC","inputBHZ.conf",default=0d0)
  call parse_input_variable(Jh,"Jh","inputBHZ.conf",default=0d0)
  ...
  call parse_input_variable(tol,"TOL","inputBHZ.conf",default=1.d-15,comment="Tolerance for non-linear equation solution.")
  ...
  allocate(Xarray(2))
  Xarray=[1d-1,1d-1]
  write(*,"(A5,4A16)")"Iter","x_1","x_2","F(x_1)","F(x_2)"
  call fsolve(bhz_mean_field,Xarray)!,tol,info)
...
contains
  ...
  function bhz_mean_field(x) result(froot)
    real(8) :: x(:)
    real(8) :: froot(size(x))
    real(8) :: Tz,Sz
    real(8) :: kx,ky
    real(8) :: A,B,D,SqrtAD,Meff,eq1,eq2,sgn1,sgn2
    integer :: ik
    integer :: iter=0
    save iter
    iter=iter+1
    ...
    do ik=1,Nktot
       ...
       !
       Meff = Mh + 2d0*Jh*Tz
       A = Meff - (cos(kx)+cos(ky))
       B = Jh*Sz
       D = lambda*(sin(kx)-xi*sin(ky))
       !
       SqrtAD= sqrt(A**2+abs(D)**2)
       !
       sgn1 = sign(1d0,B+SqrtAD)
       sgn2 = sign(1d0,B-SqrtAD)
       !
       eq1=eq1 - Jh*(sgn1 + sgn2)
       eq2=eq2 - 2d0*Jh*A/SqrtAD*(sgn1 - sgn2)
    enddo
    !
    froot(1) = eq1/Nktot + 2d0*Jh*Sz
    froot(2) = eq2/Nktot - 4d0*Jh*Tz
    !
    write(*,"(I5,4F16.9)")iter,x(1),x(2),froot(1),froot(2)
  end function bhz_mean_field
