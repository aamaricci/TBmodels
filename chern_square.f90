...
program chern_2d
  ...
  call get_Chern_Number(Hk,[Nkx,Nkx],Nso/2,Nkx/pi2*Nkx/pi2,z2)
  write(102,*)z2
...
contains
  ...
  function hk_model(kpoint,N) result(hk)
    ...
    ky=kpoint(2)
    !
    Hk(1,1) = mh - (cos(kx)+cos(ky))
    Hk(2,2) =-mh + (cos(kx)+cos(ky))
    Hk(1,2) = lambda*(sin(kx)-xi*sin(ky))
    Hk(2,1) = lambda*(sin(kx)+xi*sin(ky))
    !
    ! ek = -1d0*(cos(kx)+cos(ky))
    ! Hk = (Mh+ek)*pauli_tau_z + lambda*sin(kx)*pauli_tau_x + lambda*sin(ky)*pauli_tau_y
